...
def process_withdraw():
    ...
    cur.execute("UPDATE customers SET balance = balance - %s WHERE customer_id = %s AND balance >= %s",
                (k2, k, k2))
    cur.execute("INSERT INTO transactions (customer_id, amount, transaction_type) VALUES (%s, %s, 'withdraw')",
                (k, k2))
...
L = Label(top, text='Withdraw Funds', fg='white', bg='red', font=('Arial 25 bold'))
L.place(x=500, y=50)
...
b1 = Button(top, text='Withdraw', font=('Arial 15 bold'),command=process_withdraw)
